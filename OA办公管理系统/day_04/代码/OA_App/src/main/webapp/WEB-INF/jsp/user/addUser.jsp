	...
	<head>
		...
		<script type="text/javascript">
			...
			function validUserId(obj){
				...
				if(loginName){
					$.post("${ctx}/user/ajaxValidUser.action",{"user.userId":loginName},function(data,status){
						//后台响应正常时 status =='success'  后台响应异常时 status =='error'  
						if(status=="success"){
							if(data=="exist"){
								//用户已存在
								$.messager.alert("温馨提示","您输入的户名已存在，请重新输入！","warning");
								obj.value = "";
							}
						}
	...
	<table align="center">
		...
		<s:form action="/user/addUser.jspx" method="post" id="addUserForm" theme="simple">
			...
			<tr>
				<td>
					登&nbsp;录&nbsp;名：
					<s:textfield id="userId" name="user.userId" onblur="validUserId(this)" size="18"/>
				</td>
				<td>
					用户姓名：
