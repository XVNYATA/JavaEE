	  ...
	  $(function(){
		...
		$("#login_id").click(function(){
			...
			if($.trim(loginName)==null||$.trim(loginName)==""){
				...
				$.messager.alert('温馨提示',"您输入的用户名不合法，只能输入字母以及数字，请核实！",'warning');
				$("#userName").val("");
				$("#userName").focus();
				...
				$.messager.alert('温馨提示',"您输入的密码不合法，只能输入字母以及数字，请核实！",'warning');
				$("#password").val("");
				$("#password").focus();
			}else if(!/^[0-9]{4}$/.test(vcode)){
				$.messager.alert('温馨提示',"您输入的验证码不正确！");
				$("#vcode").focus();
			}else{
				//将表单中的数据序列化
				var params = $("#form").serialize();
				//验证通过,开始发送异步请求，验证用户输入的是否正确
				$.ajax({
					type:"post",
