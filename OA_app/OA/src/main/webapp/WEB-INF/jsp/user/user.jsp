	...
	<script type="text/javascript">
		...
		$(function(){
			...
			$.post("${ctx}/dept/ajaxLoadDept.action",function(data,status){
				//后台响应正常时status=='success' 后台响应异常时status='error'
				if(status=="success"){
					$.each(data,function(){
		...
		function userDelect(){
			...
			var boxes = $("input[id^=box_]:checked");
			if(boxes.length==0){
				$.messager.alert('温情提示',"请选择您需要删除的用户!",'warning');
			}else{
				$.messager.confirm('温情提示','是否确认删除该记录?',function(r){
					if(r){
						//获取用户选中的checkbox
						var arr = new Array();
						$.each(boxes,function(){
		...
		function userCheck(){
			...
			if(boxes.length==0){
				$.messager.alert('温馨提示',"请选择要审核的用户",'warning');
			}else if(status==-1){
				$.messager.alert('温馨提示',"请选择要审核的用户",'warning');
			}else{
				//获取用户选中的checkbox
				...
				window.location="${ctx}/user/checkUserBuIds.action?ids="+arr.join(",")+"&status="+status+"&pageModel.pageIndex=${pageModel.pageIndex}&user.name=${user.name}&user.phone=${user.phone}&user.dept.id=${user.dept.id}&user.status=${user.status}";
			}
		}
	...
	<s:form cssStyle="margin-bottom:1px;"  action="/user/selectUserByPage.action" method="post" theme="simple">
		<table>
			<tr>
				...
				<td><input type="button" name="pop_btn" id="deleteUser" value="删除" onclick="userDelect()"/></td>
				<td id="status">状态：<s:select id="checkStatus" name="user.status" list="#{-1:'请选择状态',0:'新建', 1 : '审核', 2 : '不通过', 3 : '冻结'}"/></td>
				...
				<td>部门： <select name="user.dept.id" id="deptSelect">
						<option value="${dept.name}">==请选择==</option>
				</select></td>
