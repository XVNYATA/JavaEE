		...
		<script type="text/javascript">
			$(function(){
				...
				$.post("${ctx}/user/ajaxLoadDeptAndJob.action",function(data,status){
					//后台响应正常是status=="success"  后台响应异常时是status=="error"
					if(status=="success"){
				...
				function validUserId(obj){
					...
					if(loginName){
						$.post("${ctx}/user/ajaxValidUserId.action",{"user.userId":loginName},function(data,status){
							//后台响应正常status=="success"  响应不正常status=="error"
							if(status=="success"){
								if(data=="exist"){
									//用户存在
									$.messager.alert("温情提示","用户已存在请重新输入!","warning");
									obj.value="";
								}
							}
						},"text");
	...
	<table align="center">
		...
		<s:form action="/user/addUser.jspx" method="post" id="addUserForm" theme="simple">
			...
			<tr>
				<td>
					登&nbsp;录&nbsp;名：
					&nbsp;<s:textfield id="userId" name="user.userId" size="18" onclick="validUserId(this)"/>
				</td>
				<td>
					用户姓名：
