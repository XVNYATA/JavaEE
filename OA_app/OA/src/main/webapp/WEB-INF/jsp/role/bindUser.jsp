...
<head>
	...
	$(function(){
		...
		$("#bindUser").click(function(){
			...
			if(boxes.length==0){
				$.messager.alert("温情提示","请选择需要绑定的用户",'warning');
			}else{
				 var arr = new Array();
				 $.each(boxes,function(){
					 arr.push(this.value);
				 ...
				 window.location="${ctx}/role/bindUser.action?ids="+arr.join(",")+"&role.id=${role.id}&pageModel.pageIndex=${pageModel.pageIndex}&role.id=${role.id}";
