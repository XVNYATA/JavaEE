	...
	<script type="text/javascript">
		...
		$(function(){
			...
			$("#updateRole").click(function(){
				...
				if(boxes.length==0){
					$.messager.alert('温情提示',"请选择您要修改的用户",'warning');
				}else if(boxes.length>1){
					$.messager.alert('温情提示',"一次只能修改一个用户",'warning');
				}else{
			...
			$("#deleteRole").click(function(){
				var boxes = $("input[id^=box_]:checked");
				if(boxes.length==0){
					$.messager.alert("温情提示","请选择所要删除的角色用户",'warning');
				}else{
					$.messager.confirm("温情提示","是否要删除该记录?",function(r){
						if(r){
							//获取用户选中的checked
							var arr = new Array();
							$.each(boxes,function(){
