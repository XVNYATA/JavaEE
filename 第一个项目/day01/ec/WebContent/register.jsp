...
<head>
	...
	<script type="text/javascript">
		/** 验证码看不清楚会触发这个函数 */
		var changeValidationCode = function(){
			var img = document.getElementById('JD_Verification1');
			img.src="validationCode?random=" + Math.random();
		};
		var $ = function(id){
		...
		var formOnfocus = function(id){
			...
			switch (id){
				...
				case "phone":
					...
					break;
				case "validationCode":
					$("validationCode_message").innerHTML = "请输入验证码";
					break;
			}
		};
		...
		var formOnblur = function(id){
			...
			}else if(id == "validationCode"){
				if(fieldVaue == null || fieldVaue == "" || fieldVaue.length == 0){
					$("validationCode_error").innerHTML = "您输入的验证码不符法";
				}else{
					flag = true;
				}
			}
		...
		var onRegister = function(){
			var ids = ["loginName","passWord","okPassWord","userName","address","phone","validationCode"];
...
<body>
	<!-- header部分 -->
	<div id="shortcut">
		<script type="text/javascript">header("${loginUser.name}");</script>
	﻿	<div class="nav">
			<div class="w960 center">
				<ul>
					<li><a title="首页" href="index.action">首页</a></li>
					<c:forEach items="${maxTypes}" var="type">
						<li><a title="${type.name}" href="${pageContext.request.contextPath}/index.action?typecode=${type.code}">${type.name}</a></li>
					</c:forEach>			
				</ul>
			</div>
		</div>
			...
			<form id="registerform" method="post" name="registerform" action="register">
			    ...
			    <div class="form">
			        ...
			        <div class="item">
			            ...
			            <div class="fl">
			                <input type="text" id="validationCode" style="ime-mode:disabled" name="validationCode" 
							class="text text-1"  maxlength="4" 
							onfocus="formOnfocus(this.id);" onblur="formOnblur(this.id);"/>
			                <label class="img">
			                    <img style="cursor:pointer;width:70px;height:25px;" title="验证码" onclick="changFn();" 
								 id="JD_Verification1" src="validationCode"> </label>
			                <label class="ftx23" style="height:26px;">&nbsp;看不清？
							<a href="javascript:void(0)" onclick="changeValidationCode();">换一张</a></label>
			                <span id="validationCode_ok"></span>
			                <div id="validationCode_message" style="clear:both;">${validationError }</div>
			                <label id="validationCode_error" class="null"></label>
			            </div>
			        </div>
					<!-- validationCode end -->
			        <div class="item">
